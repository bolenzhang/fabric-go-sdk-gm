...
func TestECDSAPKIXPublicKeyImportOptsKeyImporter(t *testing.T) {
	...
	ki := sm2PublicKeyImportOptsKeyImporter{}
	...
	_, err = ki.KeyImport([]byte{0}, &mocks2.KeyImportOpts{})
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "Failed converting to SM2 public key [")
...
func TestECDSAPrivateKeyImportOptsKeyImporter(t *testing.T) {
	...
	_, err = ki.KeyImport([]byte{0}, &mocks2.KeyImportOpts{})
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "Failed converting to SM2 private key [")
